...
import cryptography.ReadFolder;
...
public class User_Interface {
	...
	private ReadFolder RF = new ReadFolder();
	...
	private void initialize() {
		...
		JButton button_generated = new JButton("Random");
		button_generated.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				String Gkey = "";
				String plaintext_input = textArea_plaintext_input.getText();
				//generate the key
				for(int i = 0; i < Math.random()*plaintext_input.length(); i++) {
					Gkey = Gkey + (char)(Math.random()*26+'A');
				}
				textField_key_input.setText(Gkey);
			}
		});
		button_generated.setBounds(200, 171, 100, 23);
		panel_main.add(button_generated);
		...
		button_file_select.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	            ...
	            if (r == JFileChooser.APPROVE_OPTION)
	            {
	                ...
	                textArea_plaintext_input.setText(RF.readFolder(loc));
	            }
			}
		});
